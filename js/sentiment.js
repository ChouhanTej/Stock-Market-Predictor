/**
 * @module sentiment
 * @description News sentiment engine.
 * Live news powered by Finnhub /company-news.
 * Falls back to curated demo DB if the API call fails.
 */
...
/* ====================================================
   CURATED DEMO FALLBACK DATABASE
   ==================================================== */
const NEWS_DB = {
  AAPL: [
  ...
  'RELIANCE.NS': [
    ...
    { title: 'RIL Q4 profit rises 7% but refining margins compress',            source: 'Business Std',   age: '10h', sentiment: 'neutral',  score: 0.14 },
  ...
  'TCS.NS': [
    ...
    { title: 'TCS announces 40,000 campus hiring plan for FY26',                source: 'Business Std',   age: '12h', sentiment: 'positive', score: 0.53 },
  ...
  'IREDA.NS': [
    ...
    { title: 'Government doubles IREDA capital infusion to ₹1,500 crore',       source: 'Business Std',   age: '1d',  sentiment: 'positive', score: 0.81 },
  ...
  'YESBANK.NS': [
    ...
    { title: 'RBI scrutiny on Yes Bank advances weighs on sentiment',           source: 'Business Std',   age: '8h',  sentiment: 'negative', score: -0.52 },
...
/* ====================================================
   ...
   ==================================================== */
const POSITIVE_WORDS = ['beat','surge','record','profit','growth','rally','upgrade','buy','strong','hits','wins','bullish','rises','soars','expand','gain','revenue','deliver'];
const NEGATIVE_WORDS = ['miss','fall','recall','loss','decline','downgrade','sell','weak','concern','risk','cut','bearish','drops','slips','layoff','debt','fine','restrict','ban'];
...
function buildResult(articles, symbol) {
  ...
  const summary  = templates[Math.floor(Math.random() * templates.length)].replace(/{symbol}/g, symbol);
...
/** Demo-only sentiment (no network call) */
export function getSentiment(symbol) {
  ...
  return buildResult(articles, symbol);
}

/**
 * Fetch live company news from Finnhub and score sentiment.
 * Falls back to demo data if the network call fails or returns nothing.
 * @param {string} symbol - e.g. 'AAPL' | 'RELIANCE.NS'
 */
export async function fetchLiveNews(symbol) {
  // Finnhub uses plain US ticker for US stocks; for NSE strip .NS and search by name
  const fhSymbol = symbol.toUpperCase().endsWith('.NS')
    ? symbol.toUpperCase().replace('.NS', '')  // Finnhub doesn't have NSE news by symbol
    : symbol.toUpperCase();
  ...
  try {
    const url = `${FINNHUB_BASE}/company-news?symbol=${encodeURIComponent(fhSymbol)}&from=${fmt(from)}&to=${fmt(today)}&token=${FINNHUB_KEY}`;
    const res = await fetch(url, { signal: AbortSignal.timeout(6000) });

    if (res.ok) {
      const json = await res.json();
      if (Array.isArray(json) && json.length > 0) {
        // Take newest 8 articles
        const articles = json
          ...
          .map(item => {
            ...
            return {
              ...
              age:       formatAge(new Date(item.datetime * 1000)),
